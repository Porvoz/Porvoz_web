...
class LlamadaService:
    ...
    def ejecutar_llamadas_pendientes():
        ...
        for llamada in pendientes:
            ...
            if not numero:
                logger.warning(f"[LlamadaService] Llamada #{llamada.id}: paciente sin teléfono.")
            ...
            try:
                ...
                logger.error(f"[LlamadaService] Error ejecutando llamada #{llamada.id}: {e}")
                llamada.estado = Llamada.ESTADO_FALLIDA
                llamada.save(update_fields=["estado"])
                LlamadaService._crear_alerta_fallo(llamada, str(e))

    @staticmethod
    def registrar_respuesta(call_sid: str, transcripcion: str, como_respondio: str) -> RespuestaLlamada:
        """
        ...
        if not llamada:
            logger.warning(f"[LlamadaService] No se encontró llamada con call_sid={call_sid}")
            return None
        ...
        if como_respondio in (RespuestaLlamada.RESPUESTA_NO_ATENDIDA, RespuestaLlamada.RESPUESTA_BUZON):
    ...
    def registrar_estado_final(call_sid: str, estado_twilio: str, duracion: int = None):
        ...
        if nuevo_estado == Llamada.ESTADO_FALLIDA and not hasattr(llamada, "_alerta_creada"):
