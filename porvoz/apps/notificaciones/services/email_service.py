...
def _send_email_background(email):
    """Envía email en thread separado para no bloquear la request."""
    try:
        email.send(fail_silently=False)
    except Exception as e:
        logger.error(f"[Email] Error en background: {e}")
...
class EmailService:
    ...
    def enviar_notificacion_html(
        ...
        try:
            ...
            thread = threading.Thread(target=_send_email_background, args=(email,))
            thread.daemon = True
            thread.start()
